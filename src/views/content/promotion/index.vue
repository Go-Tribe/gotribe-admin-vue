  ...
  <el-tabs type="border-card" class="m-10">
    <el-tab-pane label="推广场景">
      <el-form size="mini" :inline="true" :model="params" class="demo-form-inline">
        ...
        <el-form-item label="推广场景描述">
          <el-input v-model.trim="params.description" clearable placeholder="推广场景描述" @clear="search" />
        </el-form-item>
        <el-form-item>
  ...
  methods: {
    ...
    batchDelete() {
      this.$confirm('此操作将永久删除, 是否继续?', '提示', {
        ...
        let msg = ''
        try {
          const { message } = await batchDeleteScene({ adSceneIds: adSceneIds.join(',') })
          msg = message
        } finally {
          this.loading = false
        }
    ...
    async singleDelete(adSceneId) {
      ...
      let msg = ''
      try {
        const { message } = await batchDeleteScene({ adSceneIds: adSceneId })
        msg = message
      } finally {
        this.loading = false
      }
